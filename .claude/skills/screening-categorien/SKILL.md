...
- **Nepnamen dragen geen geslacht én geen betekenis (2026-08-24):** `NEP_PERSONEN` en `NEP_KINDEREN` in `src/naam-anonimiseer.js` bevatten uitsluitend voornamen zonder geslachtssignaal (Robin, Sammy, Juul, Indy…). Ze worden **op volgorde** uitgedeeld, dus elke gendered naam belandt vroeg of laat bij iemand van het andere geslacht — en bij twee vrouwen of twee mannen is dat gegarandeerd.
  - Wat er misging: Liva (meisje) werd `NEP_KINDEREN[0]` = "Finn". Claude las "**Finn** stemt in met de afspraken die over **haar** zijn gemaakt", meldde terecht een tegenstrijdigheid, en na `herstelAnonObj` stond er een issue over "Liva … als jongensnaam behandeld" — over een nepnaam die in het document van de mediator niet bestaat.
  - **Tweede val, zelfde dag:** in de pool met nep-ACHTERNAMEN stond `Oud`. Een bestaande achternaam, maar ook een gewoon woord — en "oud en nieuw" staat in élk ouderschapsplan onder de feestdagen. Het terugzetten is hoofdletterongevoelig, dus "de wissel op oud en nieuw" werd "de wissel op **Lenders** en nieuw", en de mediator las een bevinding over een regeling "Lenders & Nieuw" die nergens bestond. `Oud`, `Wester` en `Kroon` zijn vervangen.
  - Voeg hier **nooit** een gendered naam toe, en ook geen naam die een gewoon woord is. Eisen: geen geslachtssignaal, ≥ 4 letters (kortere namen worden geraakt als deel van een ander woord), Nederlands-plausibel (een exotische naam suggereert een internationaal element), en niet in de andere pool. Bewaakt door `tests/unit/naam-anonimiseer.test.js`.
  - De vier bestaande afweermiddelen tegen genderbevindingen (verbod in `bevindingen.js` en `cross-doc.js`, plus twee regexfilters in `analyseer.js`) blijven staan maar zijn nú pas eerlijk: ze onderdrukken geen probleem meer dat we zelf maakten. **Verbreed die filters niet** — ze gooien de héle issuekaart weg op een woord in de titel, inclusief een goede bevinding die er toevallig naast staat.
- **Roepnaam-detectie (2026-07):** `bouwAnonMap()` detecteert roepnamen uit bestandsnamen/dossiernaam. Als "Sander" in de bestandsnaam staat bij "Alexander Lenders", wordt "sander" geregistreerd als alias voor dezelfde nep-voornaam. `naarEcht` wordt bijgewerkt: `nep.fn` → "Sander" (de roepnaam, i.p.v. de formele voornaam "Alexander"). Dit voorkomt dat Claude "Sander" als onbekende persoon ziet.
  ...
  - **Roepnaam-issue hoort bij één document (2026-08-24):** wijkt een roepnaam sterk af van de geboortenaam, dan bouwt de browser daar zélf een issue-kaart voor — het is geen bevinding van Claude. Die kaarten worden verdeeld door `bouwRoepnaamIssues()` in `src/rapport/roepnaam-issues.js`: per waarschuwing wordt gezocht in welke `documentDelen` de roepnaam als **heel woord** voorkomt, en alleen die documenten krijgen de kaart, met de passage uit dát document. Komt de roepnaam nergens voor, dan gaat de kaart naar álle documenten en logt de console een waarschuwing — een misplaatste kaart is hinderlijk, een verdwenen bevinding erger.
    - Tot 24-08-2026 was het één globale lijst die aan élk document werd geplakt, met een passage uit het eerste document dat de naam toevallig noemde. Onder het ouderschapsplan stond zo een kaart met een citaat over een bankrekening uit het convenant — een passage die in het getoonde document niet te vinden was, en dus ook niet aan te klikken.
    - Twee aanhechtpunten in `index.html`, allebei per document: `_renderProgressief()` (tijdens de analyse) en `_docResultaten` (bij het samenvoegen). Wie er een derde bijmaakt, moet óók op bestandsnaam filteren.
