...
- **Nepnamen zijn geslachtsneutraal (2026-08-24):** `NEP_PERSONEN` en `NEP_KINDEREN` in `src/naam-anonimiseer.js` bevatten uitsluitend voornamen zonder geslachtssignaal (Robin, Sammy, Juul, Indy…). Ze worden **op volgorde** uitgedeeld, dus elke gendered naam belandt vroeg of laat bij iemand van het andere geslacht — en bij twee vrouwen of twee mannen is dat gegarandeerd.
  ...
  - Voeg hier **nooit** een gendered naam toe. Eisen: geen geslachtssignaal, ≥ 4 letters (kortere namen worden geraakt als deel van een ander woord), Nederlands-plausibel (een exotische naam suggereert een internationaal element), en niet in de andere pool. Bewaakt door `tests/unit/naam-anonimiseer.test.js`.
